...
The OLTP database is initialized automatically by the official MySQL
Docker entrypoint.

The bootstrap scripts are versioned in:
...
├── 005_add_category_scd_source_attributes.sql
├── 006_add_country_scd_source_attributes.sql
└── 007_configure_cdc_user.sh
```

When MySQL starts with a new database volume, the scripts mounted under
`/docker-entrypoint-initdb.d` are executed in filename order.

The bootstrap:

* creates the `sales` database;
* creates the operational tables;
* loads the initial seed dataset;
* adds source attributes required by the SCD model;
* provisions the dedicated CDC replication user.

The expected seed row counts before later transactional changes are:
...
Initialization runs only when MySQL creates a new database volume.
Restarting an existing container does not reload the seed dataset.
...
The orders table is expected to change after initialization as normal transactional activity and CDC validation probes are introduced.
...
The warehouse uses Full Load for bootstrap or reconstruction and log-based CDC for subsequent synchronization.
