...
def test_normalize_insert_event():
    values = {
        "category_id": 900004,
        "category_name": "CDC Insert",
        "category_code": "CDC_INSERT",
        "department": "CDC Lab",
    }
    ...
    assert (
        ...
        == "binlog.000028:4151:0"
    )
    assert change_event["operation"] == "INSERT"
    assert change_event["source_schema"] == "sales"
    assert change_event["source_table"] == "categories"

    assert change_event["primary_key"] == {
        "category_id": 900004,
    }

    assert change_event["before_values"] is None
    assert change_event["after_values"] == values
    assert change_event["row_index"] == 0
    assert change_event[
    "event_timestamp"
        ] == datetime(
            2009,
...
def test_normalize_update_event():
    before_values = {
        "category_id": 900003,
    ...
    assert change_event["operation"] == "UPDATE"
...
def test_normalize_delete_event():
    values = {
        "category_id": 900003,
    ...
    assert change_event["operation"] == "DELETE"

    assert change_event["primary_key"] == {
        "category_id": 900003,
    }

    assert change_event["before_values"] == values
    assert change_event["after_values"] is None
...
def test_normalize_multiple_rows():
    ...
    assert (
        change_events[0]["event_key"]
        == "binlog.000028:5000:0"
    )

    assert (
        change_events[1]["event_key"]
        == "binlog.000028:5000:1"
    )

    assert change_events[0]["primary_key"] == {
        "category_id": 100,
    }

    assert change_events[1]["primary_key"] == {
        "category_id": 101,
    }


def test_unsupported_event_raises_type_error():
    event = Mock()
    ...
    with pytest.raises(
        TypeError,
        match="Unsupported CDC row event",
    ):
        normalize_row_event(
            event=event,
            binlog_file="binlog.000028",
            event_end_position=6000,
            primary_key_columns=("category_id",),
        )
...
def test_iter_committed_transactions():
    begin_event = Mock(spec=QueryEvent)
    ...
    xid_event = Mock(spec=XidEvent)
    xid_event.xid = 2095
    xid_event.timestamp = 1757383200
    ...
    assert transaction["commit_position"] == 578

    assert len(
        transaction["change_events"]
    ) == 1

    change_event = transaction[
        "change_events"
    ][0]
...
def test_transaction_is_not_emitted_without_commit():
    begin_event = Mock(spec=QueryEvent)
    begin_event.query = "BEGIN"
...
def test_row_event_without_transaction_raises():
    row_event = build_mock_event(
        WriteRowsEvent,
        [
            {
    ...
    with pytest.raises(
        RuntimeError,
        match="outside an active transaction",
    ):
        list(
            iter_committed_transactions(
                stream,
